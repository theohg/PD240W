...
// I2C Default Address Options (Decoded from ADCINx pins) [cite: 2675]
// #1: 0x20, #2: 0x21, #3: 0x22, #4: 0x23
#define TPS26750_I2C_ADDR_DEFAULT 0x20 

// Represents one "line item" from the charger (e.g., "15V @ 3A")
struct SourceCapability {
    uint32_t voltage_mv;
    uint32_t max_current_ma;
    bool is_pps; // Programmable Power Supply
    // For PPS: voltage_mv is the MAX voltage, current is max current
    uint32_t min_voltage_mv; // Only valid if is_pps = true
};
...
enum TPS_Reg : uint8_t {
    TPS_REG_MODE                        = 0x03, // 4 bytes [cite: 287]
    TPS_REG_CUST_USE                    = 0x06, // 8 bytes [cite: 300]
    TPS_REG_CMD1                        = 0x08, // 4 bytes [cite: 312]
    TPS_REG_DATA1                       = 0x09, // 64 bytes [cite: 327]
    TPS_REG_INT_EVENT1                  = 0x14, // 11 bytes [cite: 340]
    TPS_REG_INT_MASK1                   = 0x16, // 11 bytes [cite: 375]
    TPS_REG_INT_CLEAR1                  = 0x18, // 11 bytes [cite: 406]
    TPS_REG_STATUS                      = 0x1A, // 5 bytes [cite: 438]
    TPS_REG_POWER_PATH_STATUS           = 0x26, // 5 bytes [cite: 462]
    TPS_REG_PORT_CONFIG                 = 0x28, // 17 bytes [cite: 474]
    TPS_REG_PORT_CONTROL                = 0x29, // 4 bytes [cite: 496]
    TPS_REG_BOOT_FLAGS                  = 0x2D, // 5 bytes [cite: 519]
    TPS_REG_RX_SOURCE_CAPS              = 0x30, // 29 bytes [cite: 534]
    TPS_REG_RX_SINK_CAPS                = 0x31, // 29 bytes [cite: 547]
    TPS_REG_TX_SOURCE_CAPS              = 0x32, // 31 bytes [cite: 563]
    TPS_REG_TX_SINK_CAPS                = 0x33, // 29 bytes [cite: 589]
    TPS_REG_ACTIVE_CONTRACT_PDO         = 0x34, // 6 bytes [cite: 606]
    TPS_REG_ACTIVE_CONTRACT_RDO         = 0x35, // 4 bytes [cite: 620]
    TPS_REG_AUTONEGOTIATE_SINK          = 0x37, // 24 bytes [cite: 630]
    TPS_REG_POWER_STATUS                = 0x3F, // 2 bytes [cite: 663]
    TPS_REG_PD_STATUS                   = 0x40, // 4 bytes [cite: 674]
    TPS_REG_PD3_CONFIG                  = 0x42, // 4 bytes [cite: 704]
    TPS_REG_RX_SOP_IDENTITY             = 0x48, // 26 bytes [cite: 718]
    TPS_REG_IO_CONFIG                   = 0x5C, // 49 bytes [cite: 730]
    TPS_REG_TYPEC_STATE                 = 0x69, // 4 bytes [cite: 756]
    TPS_REG_ADC_RESULTS                 = 0x6A, // 13 bytes [cite: 770]
    TPS_REG_SLEEP_CONTROL               = 0x70, // 1 byte [cite: 783]
    TPS_REG_GPIO_STATUS                 = 0x72, // 8 bytes [cite: 797]
    TPS_REG_TX_SOURCE_CAPS_EXT          = 0x77, // 15 bytes [cite: 808]
    TPS_REG_TX_SOURCE_INFO              = 0x78, // 4 bytes [cite: 825]
    TPS_REG_TX_PPS_STATUS               = 0x7A, // 4 bytes [cite: 837]
    TPS_REG_TX_BATTERY_STATUS           = 0x7B, // 16 bytes [cite: 852]
    TPS_REG_TX_BATTERY_CAPS             = 0x7D, // 36 bytes [cite: 864]
    TPS_REG_TX_SINK_CAPS_EXT            = 0x7E, // 14 bytes [cite: 881]
    TPS_REG_LIQUID_DETECTION            = 0x98, // 11 bytes [cite: 899]
};
...
// --- STATUS Register (0x1A) Masks [cite: 442] ---
...
// --- POWER_PATH_STATUS Register (0x26) Masks [cite: 466] ---
...
// --- INT_EVENT1 Register (0x14) Masks [cite: 346] ---
...
// --- 4CC Commands (for CMD1 Register) [cite: 933-938] ---
...
#define TPS_CMD_GSrC "GSrC" // Get Source Caps
...
public:
     ...
     * The ByteCount is automatically prepended. [cite: 189]
     ...
     * @param max_caps Size of the 'caps' array (standard USB PD is max 7).
